...
def process_func(example):
  ...
  instruction = tokenizer(f"User: {example['Question'] + example['Complex_CoT']}\n\n",
                          add_special_tokens=False)  # add_special_tokens 不在开头加 special_tokens
  response = tokenizer(f"Assistant: {example['Response']}<｜end▁of▁sentence｜>", add_special_tokens=False)
